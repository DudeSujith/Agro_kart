...
class WheatPage extends StatelessWidget {
  ...
  final CollectionReference _productRef = FirebaseFirestore.instance.collection("Wheat");
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
          backgroundColor: Colors.green,
          title: Center(child: Text("Wheat Page",style:Constants.boldHeading)),
          actions: <Widget>[
            new IconButton(
                icon: Icon(
                  Icons.search,
                    ...
                    return ListView(
                      ...
                      children: snapshot.data.docs.map((document) {
                        return GestureDetector(
                          onTap: (){
                            Navigator.push(context, MaterialPageRoute(
                              builder:(context)=> WheatProduct(productId: document.id,),
                            ));
                          },
                          child: Container(
...
class WheatProduct extends StatefulWidget {
  final String productId;
  WheatProduct({this.productId});
  @override
  _WheatProductState createState() => _WheatProductState();
}

class _WheatProductState extends State<WheatProduct> {
  ...
  Future _addtocart(){
    return _firebaseservices.userID.doc(_user.uid).collection("cart").doc(widget.productId).collection('Wheat').doc().set(
        {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: Stack(
        children: [
          FutureBuilder(
              future: FirebaseFirestore.instance.collection("Wheat").doc(widget.productId).get(),
              builder: (context, snapshot){
                if(snapshot.hasError){
                  return Scaffold(
                    body: Center(
...
class WheatCart extends StatefulWidget {
  final String productId;
  WheatCart({this.productId});
  @override
  _WheatCartState createState() => _WheatCartState();
}
class _WheatCartState extends State<WheatCart> {
  Firebaseservices _firebaseservices = Firebaseservices();
  @override
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        children: [
          FutureBuilder<QuerySnapshot>(
            future: _firebaseservices.userID.doc(_firebaseservices.getuserId())
                .collection("cart").doc(widget.productId).collection('Wheat').get(),
            builder: (context, snapshot) {
              if (snapshot.hasError) {
                return Scaffold(
                  body: Center(
                ...
                return ListView(
                  ...
                  children: snapshot.data.docs.map((document) {
                    return GestureDetector(
                      ...
                      child: FutureBuilder(
                        future: FirebaseFirestore.instance.collection("Wheat").doc(document.id).get(),
...
class WheatSearch extends StatefulWidget {
  @override
  _WheatSearchState createState() => _WheatSearchState();
}

class _WheatSearchState extends State<WheatSearch> {
  SearchBar searchBar;
