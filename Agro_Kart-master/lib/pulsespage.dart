...
class pulsesCart extends StatefulWidget {
  final String productId;
  pulsesCart({this.productId});
  @override
  _pulsesCartState createState() => _pulsesCartState();
}
class _pulsesCartState extends State<pulsesCart> {
  Firebaseservices _firebaseservices = Firebaseservices();
  @override
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        children: [
          FutureBuilder<QuerySnapshot>(
            future: _firebaseservices.userID.doc(_firebaseservices.getuserId())
                .collection("cart").doc(widget.productId).collection('Pulses').get(),
            builder: (context, snapshot) {
              if (snapshot.hasError) {
                return Scaffold(
                  body: Center(
                ...
                return ListView(
                  ...
                  children: snapshot.data.docs.map((document) {
                    return GestureDetector(
                      ...
                      child: FutureBuilder(
                        future: FirebaseFirestore.instance.collection("Pulses").doc(document.id).get(),
...
class pulsesSearch extends StatefulWidget {
  @override
  _pulsesSearchState createState() => _pulsesSearchState();
}

class _pulsesSearchState extends State<pulsesSearch> {
  SearchBar searchBar;
