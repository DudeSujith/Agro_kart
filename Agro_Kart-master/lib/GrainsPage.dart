...
class GrainsPage extends StatelessWidget {
  ...
  final CollectionReference _productRef = FirebaseFirestore.instance.collection("Grains");
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
          backgroundColor: Colors.green,
          title: Center(child: Text("Grains Page",style:Constants.boldHeading)),
          actions: <Widget>[
            new IconButton(
                icon: Icon(
                  Icons.search,
                    ...
                    return ListView(
                      ...
                      children: snapshot.data.docs.map((document) {
                        return GestureDetector(
                          onTap: (){
                            Navigator.push(context, MaterialPageRoute(
                              builder:(context)=> GrainsProduct(productId: document.id,),
                            ));
                          },
                          child: Container(
...
class GrainsProduct extends StatefulWidget {
  final String productId;
  GrainsProduct({this.productId});
  @override
  _GrainsProductState createState() => _GrainsProductState();
}

class _GrainsProductState extends State<GrainsProduct> {
  ...
  Future _addtocart(){
    return _firebaseservices.userID.doc(_user.uid).collection("cart").doc(widget.productId).collection('Grains').doc().set(
        {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: Stack(
        children: [
          FutureBuilder(
              future: FirebaseFirestore.instance.collection("Grains").doc(widget.productId).get(),
              builder: (context, snapshot){
                if(snapshot.hasError){
                  return Scaffold(
                    body: Center(
...
class GrainsCart extends StatefulWidget {
  final String productId;
  GrainsCart({this.productId});
  @override
  _GrainsCartState createState() => _GrainsCartState();
}
class _GrainsCartState extends State<GrainsCart> {
  Firebaseservices _firebaseservices = Firebaseservices();
  @override
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        children: [
          FutureBuilder<QuerySnapshot>(
            future: _firebaseservices.userID.doc(_firebaseservices.getuserId())
                .collection("cart").doc(widget.productId).collection('Grains').get(),
            builder: (context, snapshot) {
              if (snapshot.hasError) {
                return Scaffold(
                  body: Center(
                ...
                return ListView(
                  ...
                  children: snapshot.data.docs.map((document) {
                    return GestureDetector(
                      ...
                      child: FutureBuilder(
                        future: FirebaseFirestore.instance.collection("Grains").doc(document.id).get(),
...
class GrainsSearch extends StatefulWidget {
  @override
  _GrainsSearchState createState() => _GrainsSearchState();
}

class _GrainsSearchState extends State<GrainsSearch> {
  SearchBar searchBar;
