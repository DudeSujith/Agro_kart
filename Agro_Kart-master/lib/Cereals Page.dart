...
class CerealsCart extends StatefulWidget {
  final String productId;
  CerealsCart({this.productId});
  @override
  _CerealsCartState createState() => _CerealsCartState();
}
class _CerealsCartState extends State<CerealsCart> {
  Firebaseservices _firebaseservices = Firebaseservices();
  @override
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        children: [
          FutureBuilder<QuerySnapshot>(
            future: _firebaseservices.userID.doc(_firebaseservices.getuserId())
                .collection("cart").doc(widget.productId).collection('Cereals').get(),
            builder: (context, snapshot) {
              if (snapshot.hasError) {
                return Scaffold(
                  body: Center(
                ...
                return ListView(
                  ...
                  children: snapshot.data.docs.map((document) {
                    return GestureDetector(
                      ...
                      child: FutureBuilder(
                        future: FirebaseFirestore.instance.collection("Cereals").doc(document.id).get(),
...
class CerealsSearch extends StatefulWidget {
  @override
  _CerealsSearchState createState() => _CerealsSearchState();
}

class _CerealsSearchState extends State<CerealsSearch> {
  SearchBar searchBar;
