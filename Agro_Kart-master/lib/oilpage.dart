...
class oilPage extends StatelessWidget {
  ...
  final CollectionReference _productRef = FirebaseFirestore.instance.collection("Oils");
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
          backgroundColor: Colors.green,
          title: Center(child: Text("Oil Product",style:Constants.boldHeading)),
          actions: <Widget>[
            new IconButton(
                icon: Icon(
                  Icons.search,
                    ...
                    return ListView(
                      ...
                      children: snapshot.data.docs.map((document) {
                        return GestureDetector(
                          onTap: (){
                            Navigator.push(context, MaterialPageRoute(
                              builder:(context)=> oilProduct(productId: document.id,),
                            ));
                          },
                          child: Container(
...
class oilProduct extends StatefulWidget {
  final String productId;
  oilProduct({this.productId});
  @override
  _oilProductState createState() => _oilProductState();
}

class _oilProductState extends State<oilProduct> {
  ...
  Future _addtocart(){
    return _firebaseservices.userID.doc(_user.uid).collection("cart").doc(widget.productId).collection('Oils').doc().set(
        {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: Stack(
        children: [
          FutureBuilder(
              future: FirebaseFirestore.instance.collection("Oils").doc(widget.productId).get(),
              builder: (context, snapshot){
                if(snapshot.hasError){
                  return Scaffold(
                    body: Center(
...
class oilCart extends StatefulWidget {
  final String productId;
  oilCart({this.productId});
  @override
  _oilCartState createState() => _oilCartState();
}
class _oilCartState extends State<oilCart> {
  Firebaseservices _firebaseservices = Firebaseservices();
  @override
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        children: [
          FutureBuilder<QuerySnapshot>(
            future: _firebaseservices.userID.doc(_firebaseservices.getuserId())
                .collection("cart").doc(widget.productId).collection('Oils').get(),
            builder: (context, snapshot) {
              if (snapshot.hasError) {
                return Scaffold(
                  body: Center(
                ...
                return ListView(
                  ...
                  children: snapshot.data.docs.map((document) {
                    return GestureDetector(
                      ...
                      child: FutureBuilder(
                        future: FirebaseFirestore.instance.collection("Oils").doc(document.id).get(),
...
class oilSearch extends StatefulWidget {
  @override
  _oilSearchState createState() => _oilSearchState();
}

class _oilSearchState extends State<oilSearch> {
  SearchBar searchBar;
