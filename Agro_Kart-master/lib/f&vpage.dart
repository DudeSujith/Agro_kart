...
class fvPage extends StatelessWidget {
  ...
  final CollectionReference _productRef = FirebaseFirestore.instance.collection("Fruits&Vegetables");
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
          backgroundColor: Colors.green,
          title: Center(child: Text("F&V Product",style:Constants.boldHeading)),
          actions: <Widget>[
            new IconButton(
                icon: Icon(
                  Icons.search,
                    ...
                    return ListView(
                      ...
                      children: snapshot.data.docs.map((document) {
                        return GestureDetector(
                          onTap: (){
                            Navigator.push(context, MaterialPageRoute(
                              builder:(context)=> fvProduct(productId: document.id,),
                            ));
                          },
                          child: Container(
...
class fvProduct extends StatefulWidget {
  final String productId;
  fvProduct({this.productId});
  @override
  _fvProductState createState() => _fvProductState();
}

class _fvProductState extends State<fvProduct> {
  ...
  Future _addtocart(){
    return _firebaseservices.userID.doc(_user.uid).collection("cart").doc(widget.productId).collection('Fruits&Vegetables').doc().set(
        {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: Stack(
        children: [
          FutureBuilder(
              future: FirebaseFirestore.instance.collection("Fruits&Vegetables").doc(widget.productId).get(),
              builder: (context, snapshot){
                if(snapshot.hasError){
                  return Scaffold(
                    body: Center(
...
class fvcart extends StatefulWidget {
  final String productId;
  fvcart({this.productId});
  @override
  _fvcartState createState() => _fvcartState();
}
class _fvcartState extends State<fvcart> {
  Firebaseservices _firebaseservices = Firebaseservices();
  @override
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        children: [
          FutureBuilder<QuerySnapshot>(
            future: _firebaseservices.userID.doc(_firebaseservices.getuserId())
                .collection("cart").doc(widget.productId).collection('Fruits&Vegetables').get(),
            builder: (context, snapshot) {
              if (snapshot.hasError) {
                return Scaffold(
                  body: Center(
                ...
                return ListView(
                  ...
                  children: snapshot.data.docs.map((document) {
                    return GestureDetector(
                      ...
                      child: FutureBuilder(
                        future: FirebaseFirestore.instance.collection("Fruits&Vegetables").doc(document.id).get(),
...
class fvSearch extends StatefulWidget {
  @override
  _fvSearchState createState() => _fvSearchState();
}

class _fvSearchState extends State<fvSearch> {
  SearchBar searchBar;
